...
class BiLSTM:
    ...
    def build_model(self, input_shape, dropout_rate, lstm_units, optimizer, loss, learning_rate):
        """
        Build the Vanilla LSTM model with BiLSTM layers.
        ...
        lstm1 = Bidirectional(LSTM(units=lstm_units, return_sequences=True))(inputs)
        lstm1 = Dropout(dropout_rate)(lstm1)
        lstm2 = Bidirectional(LSTM(units=lstm_units, return_sequences=True))(lstm1)
        lstm2 = Dropout(dropout_rate)(lstm2)
        lstm3 = Bidirectional(LSTM(units=lstm_units))(lstm2)
    ...
    def save_model(self, model_dir=None, loss=None,dataset_name=None):
        ...
        if loss is not None:
            filename = f'reg_BiLSTM_{dataset_name}_loss_{loss:.4f}.h5'
        else:
            filename = 'trained_model_bilstm_.h5'
